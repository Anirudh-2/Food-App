...
const Restaurant = () => {
  ...
  const handleFilter = (e, value) => {
    ...
    setSnackbarMessage(`Applied filter: ${e.target.name} - ${e.target.value}`);
    setSnackbarOpen(true);
  };

  // Function to handle item added to cart (trigger snackbar here)
        ...
        <section className="pt-[2rem] lg:flex relative ">
          ...
          <div className="lg:w-[80%] space-y-5 lg:pl-10">
            {menu?.menuItems.map((item) => (
              <MenuItemCard item={item} onAddToCart={handleItemAdded} />
            ))}
          </div>
        </section>
      </div>
